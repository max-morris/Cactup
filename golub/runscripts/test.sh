#!/bin/bash
# golub TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.py's mpirun (per-test
...
export CCTK_TESTSUITE_RUN_COMMAND='mpirun -np $nprocs --map-by numa:PE=@CPUS_PER_TASK@ $exe $parfile'
