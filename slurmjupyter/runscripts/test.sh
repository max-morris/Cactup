#! /bin/bash
# slurmjupyter TEST runscript (variant "test") — marked test = true in
...
# default.sh's srun (per-test task count via $nprocs).
#
...
export CCTK_TESTSUITE_RUN_COMMAND='srun -n $nprocs --cpu-bind=none --overlap $exe $parfile'
