#! /bin/bash
# expanse TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.sh's srun --mpi=pmi2
...
export CCTK_TESTSUITE_RUN_COMMAND='srun --mpi=pmi2 -n $nprocs $exe $parfile'
