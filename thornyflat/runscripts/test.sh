#! /bin/bash
# thornyflat TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.sh's mpiexec without
# the precomputed -npernode (that depends on the per-run topology; each test's
# $nprocs varies).
#
