# Crux submitscript (variant "default"), ported from simfactory2
# mdb/submitscripts/crux.sub (PBS Pro).
#
...
lines.append("#PBS -l select={0}:system=crux".format(NODES))
lines.append("#PBS -l place=scatter")
if CHAINED_JOB_ID:
    lines.append("#PBS -W depend=afterany:{0}".format(CHAINED_JOB_ID))
lines.append("#PBS -V")
lines.append("#PBS -l filesystems=home:eagle:grand")
