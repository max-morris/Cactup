#! /bin/bash
# qb TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.sh's mvapich2 mpirun
...
export CCTK_TESTSUITE_RUN_COMMAND='mpirun -np $nprocs /bin/env MV2_ENABLE_AFFINITY=0 OMP_NUM_THREADS=@CPUS_PER_TASK@ $exe $parfile'
