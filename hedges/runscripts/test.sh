#!/bin/bash
# hedges TEST runscript (variant "test") — marked test = true in meta.toml
...
# hedges' runscript (generic-mpi.run) always launches through mpirun, so the
# testsuite launcher is pinned to mpirun as well.
