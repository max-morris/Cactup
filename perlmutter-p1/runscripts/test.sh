#! /bin/bash
# perlmutter-p1 TEST runscript (variant "test") — marked test = true in
...
# Cactus flesh harness launch each test (design §11.6). The launcher is
# default.sh's srun with the per-test task count made explicit via $nprocs
# (the default script relies on the SLURM allocation instead).
#
...
export CCTK_TESTSUITE_RUN_COMMAND='srun -n $nprocs $exe $parfile'
