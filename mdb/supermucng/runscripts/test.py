# supermucng TEST runscript (variant "test") — marked test = true in
# meta.toml (design §11.2); .py form of the earlier test.sh. Instead of
# launching a parfile like runscripts/default.py, it drives
# `make <config>-testsuite` and lets the Cactus flesh harness launch each
# test (design §11.6). The launcher mirrors default.py's mpiexec (per-test
# task count via $nprocs).
#
