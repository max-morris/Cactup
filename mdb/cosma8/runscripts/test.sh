#! /bin/bash
# cosma8 TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.sh's mpirun.
#
...
export CCTK_TESTSUITE_RUN_COMMAND='mpirun -n $nprocs $exe $parfile'
