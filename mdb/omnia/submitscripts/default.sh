#! /bin/bash
# omnia submitscript (variant "default"), ported from simfactory2 generic.sub
# (the old ini's `submitscript = generic.sub`; the new format has no
# cross-machine file sharing, so this machine carries its own copy).
