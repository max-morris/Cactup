#! /bin/bash
# LEONARDO DCGP TEST runscript (variant "test") — marked test = true in
# meta.toml (design §11.2). Instead of launching a parfile like
# runscripts/default.py, it drives `make <config>-testsuite` and lets the
# Cactus flesh harness launch each test (design §11.6). The launcher mirrors
# default.py's srun (per-test task count via $nprocs). A plain .sh suffices
# here — none of default.py's computed float output is needed.
#
