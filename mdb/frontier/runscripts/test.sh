#! /bin/bash
# Frontier TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.sh's srun (per-test
# task count via $nprocs, one MI250X GCD per rank via --gpus=$nprocs).
#
...
export CCTK_TESTSUITE_RUN_COMMAND='srun --ntasks=$nprocs --gpus=$nprocs --gpu-bind=closest $exe $parfile'
