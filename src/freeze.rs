    ...
    FROZEN
        .get_or_init(|| match std::env::current_exe() {
            Ok(exe) => {
                ...
                freeze(&exe, stamp).display().to_string()
            }
...
/// dev build, which runs itself).
fn freeze(exe: &Path, stamp: Option<Stamp>) -> PathBuf {
    let Some(stamp) = stamp else { return exe.to_owned() };
    match versioned_copy(exe, stamp.id) {
        Ok(path) => path,
        Err(e) => {
...
/// same directory, copied from `exe` (temp file, mode 0755, rename) unless
/// it is already there.
fn versioned_copy(exe: &Path, id: &str) -> Res<PathBuf> {
    ...
    let mut source =
        std::fs::File::open(exe).with_context(|| format!("Failed to open {}", exe.display()))?;
...
mod tests {
    ...
    fn a_versioned_binary_runs_itself() {
        ...
        assert_eq!(freeze(exe, Some(STAMP)), exe);
    }
    ...
    fn a_plain_binary_gets_a_versioned_copy_that_is_then_reused() {
        ...
        let frozen = freeze(&exe, Some(STAMP));
        ...
        assert_eq!(freeze(&exe, Some(STAMP)), frozen);
