#! /bin/bash
# Sciama TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.py's openmpi mpirun
# without the precomputed hostfile/npernode (those depend on the per-run
# topology; each test's $nprocs varies). A plain .sh suffices here — none of
# default.py's computed float output is needed.
#
...
export CCTK_TESTSUITE_RUN_COMMAND='${MPIDIR}/bin/mpirun -v --mca btl openib,self --mca mpi_leave_pinned 0 -np $nprocs $exe $parfile'
