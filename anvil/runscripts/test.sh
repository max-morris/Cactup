#! /bin/bash
# anvil TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.sh's mpirun (the
# upstream warned against srun on anvil: incorrect thread bindings).
#
