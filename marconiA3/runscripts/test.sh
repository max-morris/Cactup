#! /bin/bash
# MARCONI A3 TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher is default.py's mpirun with the
# per-test task count made explicit via $nprocs (the default script relies on
# the SLURM allocation instead). A plain .sh suffices here — none of
