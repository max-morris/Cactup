#! /bin/bash
# minerva TEST runscript (variant "test") — marked test = true in meta.toml
...
# each test (design §11.6). The launcher mirrors default.sh's mpiexec.hydra
...
export CCTK_TESTSUITE_RUN_COMMAND='mpiexec.hydra -genv MALLOC_MMAP_MAX_=0 -genv MALLOC_TRIM_THRESHOLD_=536870912 -n $nprocs $exe $parfile'
